...
const Step6BreathingChallenge = ({ userProfile, onUpdateProfile, onNext, onBack, devInitialState }: Step6BreathingChallengeProps) => {
  ...
  const [countdownNumber, setCountdownNumber] = useState(3); // Número do countdown (3, 2, 1)
  const [isFirstInhale, setIsFirstInhale] = useState(true); // Para começar do ponto 0
  const [phaseStartTime, setPhaseStartTime] = useState<number>(0); // Timestamp do início da fase para animação suave
  ...
  // Countdown effect: PREPARE-SE + número (3, 2, 1) JUNTOS - countdown de 3 segundos
  useEffect(() => {
    if (phase !== 'countdown') return;

    // Começar com número 3 (PREPARE-SE sempre visível em cima)
    ...
    const interval = setInterval(() => {
      setCountdownNumber(prev => {
        if (prev <= 1) {
          ...
          setPhaseStartTime(Date.now()); // Marcar início da fase para animação suave
          return 1;
        }
        return prev - 1;
      });
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      setTimer(prev => {
        if (prev <= 1) {
          // Transição entre fases - resetar phaseStartTime para animação do anel começar em 0%
  ...
  const handleStart = () => {
    setPhase('countdown');
    setCountdownNumber(3); // Começar em 3
    setTimeElapsed(0);
    setBreatheCount(0);
    setPoints(0);
    setShowSkip(false);
  ...
  if (phase === 'challenge') {
    ...
    return (
      <div className="min-h-screen bg-background flex flex-col p-4">
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="relative flex items-center justify-center">
            {/* Anel Luminoso SVG - FORA do círculo - ANIMAÇÃO SUAVE CSS */}
            <svg 
              className="absolute pointer-events-none"
            ...
            >
              ...
              <circle
                ...
                strokeWidth={ringStrokeWidth}
              />
              {/* Anel de progresso luminoso - ANIMAÇÃO SUAVE usando CSS animation */}
              <circle
                cx={ringSize / 2}
                cy={ringSize / 2}
                r={ringRadius}
                ...
                style={{ 
                  animation: `ringProgress ${phaseDuration}s linear forwards`,
                  transform: 'rotate(-90deg)',
                  transformOrigin: 'center'
                }}
                key={`${breathPhase}-${phaseStartTime}`} /* Key para resetar animação a cada fase */
              />
              <style>{`
